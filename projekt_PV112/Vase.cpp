...
Vase::Vase(Movement* m){
	this->m = m;
	vase = new ObjLoader();
	vase = vase->loadObject("vase");
	...
	vector<Vector3f> vertices = vase->GetVertices();
	vector<Vector3f> normals = vase->GetNormals();
	vector<Vector2f> textures = vase->GetTextures();
	vector<Triangle> triangles = vase->GetTriangles();

	glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, vase_color);
...
void Vase::draw(bool first_render, GLuint texture){
	if (first_render){
		GLfloat mat[16];
...
void Vase::setMinMax(float x, float z){
	if (x < x_min){
		x_min = x;
	}
	if (x > x_max){
